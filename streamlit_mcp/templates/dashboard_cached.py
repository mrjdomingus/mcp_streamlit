...
st.set_page_config(
    page_title="Sales Dashboard", page_icon="📊", layout="wide", initial_sidebar_state="expanded"
)
...
def calculate_metrics(df):
    ...
    sales_change = (
        (
            (last_30_days["sales"].sum() - prev_30_days["sales"].sum())
            / prev_30_days["sales"].sum()
            * 100
        )
        if prev_30_days["sales"].sum() > 0
        else 0
    )
...
categories = st.sidebar.multiselect(
    "Categories", options=data["category"].unique(), default=data["category"].unique()
)

# Region filter
regions = st.sidebar.multiselect(
    "Regions", options=data["region"].unique(), default=data["region"].unique()
)
...
with col2:
    st.metric(
        "Total Customers", f"{metrics['total_customers']:,.0f}", help="Number of customers served"
    )

with col3:
    st.metric("Total Orders", f"{metrics['total_orders']:,.0f}", help="Number of orders placed")

with col4:
    st.metric(
        "Avg Order Value", f"${metrics['avg_order_value']:,.2f}", help="Average value per order"
    )
...
with col1:
    ...
    fig_trend = px.line(daily_sales, x="date", y="sales", title="Daily Sales Over Time")
    fig_trend.update_layout(xaxis_title="Date", yaxis_title="Sales ($)", hovermode="x unified")
...
with col2:
    ...
    fig_category = px.pie(
        category_sales, values="sales", names="category", title="Sales Distribution by Category"
    )
...
with col1:
    ...
    fig_region = px.bar(
        region_sales, y="region", x="sales", orientation="h", title="Regional Sales Performance"
    )
    fig_region.update_layout(xaxis_title="Sales ($)", yaxis_title="Region")
...
with col2:
    ...
    daily_agg = (
        filtered_data.groupby("date").agg({"orders": "sum", "customers": "sum"}).reset_index()
    )
    ...
    fig_orders.update_layout(xaxis_title="Customers", yaxis_title="Orders")
...
with tab2:
    ...
    st.dataframe(
        filtered_data[["sales", "customers", "orders"]].describe(), use_container_width=True
    )

with tab3:
    st.markdown("### Top 10 Days by Sales")
    top_days = filtered_data.nlargest(10, "sales")[
        ["date", "sales", "customers", "orders", "category", "region"]
    ]
